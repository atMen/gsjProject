...
import customer.tcrj.com.zsproject.adapter.hRecycleviewAdapter;
...
public class zzfcFregment extends BaseFragment implements BaseQuickAdapter.OnItemClickListener, hRecycleviewAdapter.OnItemClickLitener {
...
//                getData(pageNum);
        ...
        for(int i = 0;i < 10;i++){
            tpxwInfo.ListinfoBean s = new tpxwInfo.ListinfoBean(i+"标题");
            beanList.add(s);
        }

        LinearLayoutManager ms= new LinearLayoutManager(mContext);
        ms.setOrientation(LinearLayoutManager.HORIZONTAL);// 设置 recyclerview 布局方式为横向布局
        hRecyclerView.setLayoutManager(ms); //给RecyClerView 添加设置好的布局样式
        hRecyclerView.setAdapter(hAdapter = new hRecycleviewAdapter(beanList, mContext)); //对recyclerview 添加数据内容
        hAdapter.setOnItemClickLitener(this);
...
//    }
    ...
    public void OnItemClick(int position) {
        hAdapter.setSelection(position);
    }
}
